<div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6 mb-8" x-data="{ showFilters: false }">
        <div class="flex flex-col lg:flex-row gap-4">
            ...
            <div class="flex-1 flex flex-wrap gap-3" :class="{ 'hidden lg:flex': !showFilters }">
                <!-- Destination Filter -->
                <div class="relative">
                    <select wire:model.live="destination" class="appearance-none pl-4 pr-10 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white text-sm min-w-[160px]">
            ...
            <div class="flex items-center gap-4" :class="{ 'hidden lg:flex': !showFilters }">
                ...
                <span class="text-gray-500 text-sm whitespace-nowrap">
                    <span wire:loading.remove wire:target="destination, category, duration, priceRange, sortBy">
                        {{ $tours->total() }} tour{{ $tours->total() !== 1 ? 's' : '' }} found
                    </span>
                    <span wire:loading wire:target="destination, category, duration, priceRange, sortBy" class="flex items-center gap-2">
        ...
        <!-- Active Filters Tags -->
        @if($hasFilters && ($destination || $category || $duration || $priceRange))
            <div class="mt-4 pt-4 border-t flex flex-wrap gap-2">
                ...
                @if($destination)
                    <span class="inline-flex items-center gap-1 px-3 py-1 bg-primary-100 text-primary-700 rounded-full text-sm">
                        {{ $destinations->firstWhere('slug', $destination)?->name ?? $destination }}
                        <button wire:click="$set('destination', '')" class="hover:text-primary-900">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                        </button>
    ...
    <!-- Tours Grid -->
    <div wire:loading.class="opacity-50" wire:target="destination, category, duration, priceRange, sortBy, gotoPage">
        @if($tours->count())
        ...
        @else
            <div class="text-center py-16 bg-white rounded-xl shadow-md">
                ...
                <p class="text-gray-500 mb-6">Try adjusting your filters or browse all our tours</p>
